...
{
	colliders_.remove_if(std::bind1st((std::equal_to<Collider *>()), collider)); //crash candidate
	delete collider;
}
...
void Collision::DoCollisions(Game *game)
{
		...
		++colliderAIt)
	{
		...
		for (++colliderBIt; colliderBIt != end; ++colliderBIt)
		{
			...
			{
				currentCollisions.push_back(std::pair<Collider*,Collider*>(colliderA, colliderB));
			}
		}
	}
	for (CollisionMap::iterator collisionIter = currentCollisions.begin(); collisionIter != currentCollisions.end(); collisionIter++) {
		game->DoCollision(collisionIter->first->entity, collisionIter->second->entity);
	}
}
